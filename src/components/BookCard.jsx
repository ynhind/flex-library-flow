...
const BookCard = ({ book }) => {
  ...
  const renderRatingStars = (rating) => {
    ...
    for (let i = 0; i < fullStars; i++) {
      stars.push(
        <Star key={`full-${i}`} size={14} className="fill-amber-500 text-amber-500" />
      );
    }
  ...
  return (
    ...
    >
      <div className="relative aspect-[2/3] overflow-hidden">
        ...
        <button 
          className={`absolute top-2 right-2 p-2 bg-white/80 rounded-full transition-all duration-300 ${isFavorite ? 'text-red-500' : 'text-gray-500'}`}
      ...
      <div className="p-4">
        <Link to={`/book/${id}`} className="block">
          <h3 className="font-serif font-medium text-lg text-stone-800 mb-1 truncate">{title}</h3>
        ...
        <div className="flex justify-between items-center">
          <span className="font-medium text-stone-800">${price.toFixed(2)}</span>
